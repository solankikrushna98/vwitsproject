  ...
  <body>
	...
	if(username == null){
		out.println("Session Invalid");
			...
			List<Student> multiple = (List)request.getAttribute("list");
      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="myNavbar">
          <ul class="nav navbar-nav">
		     ...
		     <li class="nav-item active"><a class="nav-link" href="DisplayResult">Display Results</a></li>
		 ...
		 <table class="table">
		  ...
		  <tbody>
		    ...
		    <% for(int i=0; i<multiple.size(); i++){ %>
		      <th scope="row"><%=multiple.get(i).getStudentid()%></th>
		      <td><%=multiple.get(i).getName()%></td>
		      <td><%=multiple.get(i).getUsername()%></td>
		      <%if( multiple.get(i).getResult() == -1){ %>
		      	<td>Not Attempted</td>
		      <% }else{ %>
		      <td><%=multiple.get(i).getResult()%></td>
		      <% } %>
		    </tr>
		    <% }%>
		  </tbody>
